...
export async function registroProfesionalAction(
  ...
  try {
    ...
    // Determinar la URL base de redirección de confirmación
    const headersList = await headers();
    const host = headersList.get('x-forwarded-host') || headersList.get('host') || 'portal-steffen.vercel.app';
    const proto = headersList.get('x-forwarded-proto') || 'https';
    const appOrigin = process.env.NEXT_PUBLIC_APP_URL || `${proto}://${host}`;
    const emailRedirectUrl = `${appOrigin}/auth/callback?next=/cuenta-confirmada`;
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.cliente.create({
          data: {
            ...
            beneficioActual: '20% OFF primer pedido',
          },
        });
      });
    } catch (prismaError) {
    ...
    return {
      success: true,
      redirectTo: `/registro/verifica-email?email=${encodeURIComponent(email)}`,
    };
  } catch (error: any) {
  ...
  try {
    ...
    if (authError || !authData.user) {
      if (
        ...
        return {
          error: 'Todavía no confirmaste tu email. Revisá tu bandeja de entrada para activar tu cuenta.',
        };
      }
    ...
    try {
      ...
      const redirectCustom = (formData.get('redirect') as string)?.trim();
      if (redirectCustom && redirectCustom.startsWith('/') && !redirectCustom.startsWith('//')) {
        return {
          success: true,
          redirectTo: redirectCustom,
        };
      }
      ...
      // Si el cliente está ACTIVO y todavía no vio la alerta de bienvenida/aprobación profesional
      const noVioAlertaAprobacion = usuario?.cliente && !usuario.cliente.alertaAprobacionVista;
      if (acabaDeSerAprobado || noVioAlertaAprobacion) {
        return {
          success: true,
...
/**
 * Marca como vista la alerta de cuenta aprobada y habilitación de precios profesionales
 * para que no vuelva a aparecer en inicios de sesión posteriores.
 */
export async function marcarAlertaAprobacionVistaAction(): Promise<{ success: boolean }> {
  try {
    const supabase = await createServerSupabaseClient();
    const {
      data: { user },
    ...
    await prisma.cliente.update({
      where: { id: usuario.cliente.id },
      data: { alertaAprobacionVista: true },
    });

    return { success: true };
  } catch (error) {
    console.error('Error al marcar alerta de aprobación como vista:', error);
    return { success: false };
  }
}
